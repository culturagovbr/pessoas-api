...
public class PessoaHadoopDTODeserializer implements JsonDeserializer<PessoaHadoop> {

	public PessoaHadoop deserialize(JsonElement jsonElement, Type type, JsonDeserializationContext jsonDeserializationContext)
		...
		if (jsonElement.isJsonObject() || !jsonElement.getAsString().equalsIgnoreCase("Not")) {
			...
			String cpf = decode(row.get("key").getAsString());
			...
			return new PessoaHadoop(cpf, colunas);
